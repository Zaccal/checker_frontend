...
import type { HookTarget } from '@/lib/reactuse'

import { getElement, isTarget } from '@/lib/reactuse'
...
export const useHotkeys = ((...params: any[]) => {
  ...
  const options = (target ? params[3] : params[2]) as
    | UseHotkeysOptions
    | undefined
  ...
  const onKeyDown = useEvent((event: KeyboardEvent) => {
    ...
    const isMatch = hotkeysList.some(hotkey =>
      isHotkeyMatch(hotkey, updatedKeys),
    )
    if (!isMatch) return
